...
class StripeService:
    ...
    def create_checkout_session(self, **kwargs):
        ...
        try:
            ...
            # Handle product and pricing
            if product_id:
                try:
                    product = Product.objects.get(id=product_id, active=True)
                    if payment_type == 'subscription' and product.product_type != 'subscription':
                        raise ValueError(f"Product {product_id} is not configured for subscriptions")
                    ...
                    if payment_type == 'subscription':
                        if not product.stripe_price_id:
                            ...
                            stripe_product, stripe_price = self._create_stripe_subscription_product(product)
            ...
            if payment_type == 'subscription':
                checkout_session = self._create_subscription_checkout_session(
                    product, customer, metadata, success_url, cancel_url, trial_period_days
                )
            else:
                checkout_session = self._create_one_time_checkout_session(
                    amount, currency, product_name, customer, metadata, success_url, cancel_url
                )
            ...
            logger.info(f"Checkout session created: {checkout_session.id} (type: {payment_type}) with search_uid: {search_uid}")
    ...
    def _create_subscription_checkout_session(self, product, customer, metadata, success_url, cancel_url, trial_period_days=None):
    ...
    def _create_one_time_checkout_session(self, amount, currency, product_name, customer, metadata, success_url, cancel_url):
    ...
    def manage_subscription(self, subscription_id, action, **kwargs):
        ...
        try:
            if action == 'cancel':
                return self._cancel_subscription(subscription_id, kwargs.get('cancel_at_period_end', True))
            elif action == 'pause':
            ...
            logger.error(f"Error managing subscription {subscription_id}: {str(e)}", exc_info=True)
            raise
    ...
    def _handle_subscription_created(self, subscription_data):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if not customer:
                    logger.warning(f"Customer not found for subscription: {subscription_id}")
                    return

                # Find associated payment
                payment = Payment.objects.filter(stripe_subscription_id=subscription_id).first()
            ...
            logger.error(f"Error processing subscription creation: {str(e)}", exc_info=True)
            raise
    ...
    def _handle_invoice_payment_succeeded(self, invoice_data):
        ...
        try:
            ...
            logger.error(f"Error processing invoice payment success: {str(e)}", exc_info=True)
            raise
    ...
    def _handle_invoice_payment_failed(self, invoice_data):
        ...
        try:
            ...
            logger.error(f"Error processing invoice payment failure: {str(e)}", exc_info=True)
            raise
