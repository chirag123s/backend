...
class MyGarage(models.Model):
    """User's garage to store their vehicles and search preferences"""
    id = models.AutoField(primary_key=True)
    user_id = models.CharField(max_length=255)  # Auth0 user ID
    user_email = models.CharField(max_length=255, null=True, blank=True)
    
    # Search preferences (copied from CarSearchLog)
    save_money = models.BooleanField(default=False)
    greener_car = models.BooleanField(default=False)
    good_all_rounder = models.BooleanField(default=False)
    budget = models.DecimalField(max_digits=10, decimal_places=2)
    ...
    # Metadata
    original_search_uid = models.CharField(max_length=50, null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.user_email} - {self.make} {self.model} ({self.year})"


class UserSubscription(models.Model):
    """Track user subscriptions and link them to search sessions"""
    id = models.AutoField(primary_key=True)
    user_id = models.CharField(max_length=255)  # Auth0 user ID
    user_email = models.CharField(max_length=255)
    
    # Payment reference
    payment_uuid = models.CharField(max_length=255, null=True, blank=True)
    
    # Search session reference  
    search_uid = models.CharField(max_length=50, null=True, blank=True)
